...
/** Runtime configuration for the cloudwatch-metrics lambda, sourced from environment variables set by the CDK stack.
  *
  *   - `DRY_RUN` (default `true`): when true, metrics are logged but not written to CloudWatch.
  *   - `CONFIG_BUCKET`: S3 bucket holding `security-hq.conf`.
  *   - `CONFIG_KEY`: S3 key for `security-hq.conf`.
  *   - `REGION` (default `eu-west-1`): primary region for the own-account clients and config bucket.
  */
case class Settings(
    dryRun: Boolean,
    configBucket: String,
