...
class CredentialsReportDisplayTest extends AnyFreeSpec with Matchers {
  ...
  "display logic" - {
    ...
    "machine report status green when key1 enabled" in {
      val machineCred =
        cred.copy(passwordEnabled = None, passwordLastUsed = None, accessKey2Active = false, accessKey1Active = true)
      machineReportStatus(machineCred) shouldBe Green
    }

    "machine report status green when key2 enabled" in {
      val machineCred =
        cred.copy(passwordEnabled = None, passwordLastUsed = None, accessKey2Active = true, accessKey1Active = false)
      machineReportStatus(machineCred) shouldBe Green
    }

    "check machine report status amber when no key enabled" in {
      val machineCred =
        cred.copy(passwordEnabled = None, passwordLastUsed = None, accessKey1Active = false, accessKey2Active = false)
    ...
    "check human report status green when mfa active, no active access key and google username tag is present and value is correct" in {
      val humanCred = cred.copy(
        accessKey1Active = false,
        accessKey2Active = false,
        mfaActive = true,
        tags = List(Tag(USERNAME_TAG_KEY, "amina.adewusi"))
      )
    ...
    "check human report status amber when google username tag value does not contain a full stop" in {
      val humanCred = cred.copy(
        tags = List(Tag(USERNAME_TAG_KEY, "aminaadewusi")),
        accessKey1Active = false,
        accessKey2Active = false
      )
      humanReportStatus(humanCred) shouldBe Amber(Seq(MissingUsernameTag))
    ...
    "check human report status amber when google username tag value is empty" in {
      val humanCred =
        cred.copy(tags = List(Tag(USERNAME_TAG_KEY, "")), accessKey1Active = false, accessKey2Active = false)
      humanReportStatus(humanCred) shouldBe Amber(Seq(MissingUsernameTag))
    }
    ...
    "check credential display report for human user" in {
      ...
      val humanUser = HumanUser(
        cred.user,
        cred.mfaActive,
        AccessKey(AccessKeyEnabled, Some(now.minusDays(4))),
  ...
  "sortAccountsByReportSummary" - {
    ...
    val machineAmber = MachineUser(
      "machineAmber",
      AccessKey(AccessKeyDisabled, None),
      AccessKey(NoKey, None),
      Amber(),
      Some(1),
      None,
    ...
    val reportAllGreen = CredentialReportDisplay(
      now,
      humanUsers = Seq(humanGreen, humanGreen),
      machineUsers = Seq(machineGreen, machineGreen)
    )
    val reportSomeWarnings = CredentialReportDisplay(
      now,
      humanUsers = Seq(humanGreen, humanAmber, humanGreen),
      machineUsers = Seq(machineAmber, machineGreen, machineGreen)
    )
    val reportMediumConcern = CredentialReportDisplay(
      now,
      humanUsers = Seq(humanGreen, humanAmber, humanRed, humanAmber),
      machineUsers = Seq(machineAmber, machineAmber, machineGreen)
    )
    val reportBadScenario = CredentialReportDisplay(
      now,
      humanUsers = Seq(humanGreen, humanAmber, humanRed, humanAmber),
      machineUsers = Seq(machineAmber, machineAmber, machineGreen)
    )
    val reportWorstCase = CredentialReportDisplay(
      now,
      humanUsers = Seq(humanRed, humanAmber, humanRed, humanGreen),
      machineUsers = Seq(machineAmber, machineGreen)
    )
  ...
  "reportStatusSummary" - {
    ...
    "returns individual counts for each report status" in {
      ...
      val report = CredentialReportDisplay(
        now,
        humanUsers = Seq(humanRed, humanGreen),
        machineUsers = Seq(machineAmber, machineBlue, machineGreen, machineAmber, machineBlue, machineBlue)
      )
  ...
  "sortUsersByReportSummary" - {
    ...
    "will sort according to username if the ReportStatuses are equal" in {
      ...
      sortUsersByReportSummary(report).machineUsers shouldEqual Seq(
        machineGreenA,
        machineGreenA,
        machineGreenB,
        machineGreenB
      )
      sortUsersByReportSummary(report).humanUsers shouldEqual Seq(
        humanAmberA,
        humanAmberA,
        humanAmberB,
        humanAmberB,
  ...
  "exposedKeysSummary" - {
    ...
    val exposedKeys = List(
      ExposedIAMKeyDetail(
        "key-id",
        "username",
        "bad fraud",
        "2345671234",
        "2017-29-09T11:32:04Z",
        "the internet",
        "Soon",
        "EC2"
      )
