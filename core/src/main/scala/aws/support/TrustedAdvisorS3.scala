...
object TrustedAdvisorS3 {
  ...
  )(implicit ec: ExecutionContext): Attempt[List[(AwsAccount, Either[FailedAttempt, List[BucketDetail]])]] = {
    Attempt.Async.Right {
  ...
  private def getBucketReport(
      client: AwsClient[SupportAsyncClient]
  )(implicit ec: ExecutionContext): Attempt[TrustedAdvisorDetailsResult[BucketDetail]] = {
...
//  it to disk in its data centers and decrypts it when you download the object
  private def addEncryptionStatus(bucket: BucketDetail, account: AwsAccount, clients: AwsClients[S3Client])(implicit
      ec: ExecutionContext
  ): Attempt[Option[BucketDetail]] = {
  ...
  ): Option[BucketDetail] = status match {
  ...
  private def publicBucketsForAccount(
      ...
      taClients: AwsClients[SupportAsyncClient],
      s3Clients: AwsClients[S3Client]
  )(implicit ec: ExecutionContext): Attempt[List[BucketDetail]] = {
    for {
    ...
    detail.metadata.asScala.toList match {
      ...
      case metadata =>
        Attempt.Left {
          utils.attempt
            .Failure(
              s"Could not parse S3 Bucket report from TrustedAdvisorResourceDetail with metadata $metadata",
              "Could not parse public S3 Buckets",
              500
            )
            .attempt
        }
