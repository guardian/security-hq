...
/** Represents a value that will need to be calculated using an asynchronous computation that may fail.
  */
case class Attempt[A] private (underlying: Future[Either[FailedAttempt, A]]) extends LazyLogging {
  ...
  /** If there is an error in the Future itself (e.g. a timeout) we convert it to a Left so we have a consistent error
    * representation. Unfortunately, this means the error isn't being handled properly so we're left with just the
    * information provided by the exception.
    *
    * Try to avoid hitting this method's failure case by always handling Future errors and creating a suitable failure
    * instance for the problem.
    */
...
object Attempt {
  ...
  /** Changes generated `List[Attempt[A]]` to `Attempt[List[A]]` via provided traversal function (like
    * `Future.traverse`).
    *
    * This implementation returns the first failure in the resulting list, or the successful result.
    */
  ...
  /** Using the provided traversal function, sequence the resulting attempts to `Attempt[List[(A, B)]]` where the first
    * element of each tuple is the value passed to the function f in order to generate the second element
    *
    * This implementation returns the first failure in the resulting list, or the successful result.
    */
  ...
  /** Traverses the given list `List[A]` with the function f, A => Attempt[List[B]]` and flattens the generated result
    * into Attempt[List[B]]` This implementation returns the first failure in the resulting list, or the successful
    * result.
    */
  ...
  /** Using the provided traversal function, sequence the resulting attempts into a list that preserves failures.
    *
    * This is useful if failure is acceptable in part of the application.
    */
  def traverseWithFailures[A, B](
      as: List[A]
  )(f: A => Attempt[B])(implicit ec: ExecutionContext): Attempt[List[Either[FailedAttempt, B]]] = {
    sequenceWithFailures(as.map(f))
  }

  /** Using the provided traversal function, sequence the resulting attempts into a list that:
    ...
    */
  def labelledTraverseWithFailures[A, B](
      as: List[A]
  )(f: A => Attempt[B])(implicit ec: ExecutionContext): Attempt[List[(A, Either[FailedAttempt, B])]] = {
  ...
  def fromFuture[A](
      future: Future[A]
  )(recovery: PartialFunction[Throwable, FailedAttempt])(implicit ec: ExecutionContext): Attempt[A] = {
    Attempt {
      future
        .map(scala.Right(_))
        .recover { case t =>
  ...
  /** Asyncronous versions of the Attempt Right/Left helpers for when you have a Future that returns a good/bad value
    * directly.
    */
  object Async {
    ...
    /** Create an Attempt from a known failure in the future. For example, if a piece of logic fails but you need to
      * make a Database/API call to get the failure information.
      */
  ...
  /** @see
    *   https://stackoverflow.com/questions/16359849/scala-scheduledfuture
    */
  object Delayed {
